...
export function InsuranceReportModal({
  open,
  text,
  onClose,
}: {
  ...
  async function copy() {
    ...
    try {
      ...
      setStatus("הועתק! עכשיו אפשר להדביק בכל אפליקציה אחרת.");
    } catch {
      const ok = document.execCommand && document.execCommand("copy");
      setStatus(
        ok
          ? "הועתק! עכשיו אפשר להדביק בכל אפליקציה אחרת."
  ...
  return (
    <Modal open={open} onClose={onClose} maxWidthClassName="max-w-lg">
      <h3 className="mb-1 text-lg font-bold text-teal-800 dark:text-teal-400">דוח השוואת ביטוחים</h3>
      ...
      <textarea
        ...
        value={text}
        dir="rtl"
        className="min-h-[280px] flex-1 resize-y whitespace-pre-wrap rounded-lg border border-black/10 bg-neutral-50 p-3 text-sm outline-none dark:border-white/10 dark:bg-neutral-950"
      />
